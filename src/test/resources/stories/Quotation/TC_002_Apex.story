Scenario:  TC_002_Apex TC2S1 Apex Test Case Motor - Motor Private - Comprehensive (Saudi Riyals)

Given [Navigation] I navigate to 'p:amanApex'
And [Input] I login as 'p:adminUsername' with password 'p:password'
And [Progress] I wait for '5' sec
When [Input] I click on 'QuotationCardDashboard'
And [Assertion] Verify 'QuotationsTitle' equals 'Quotations' text
And [Input] I click on 'SPWaziriyahMrorrCard'
And [Input] I click on 'NewQuotationButton'
And [Input] I click on 'MotorCard'
And [Input] I click on 'MotorPrivateCard'
And [Input] I click on 'ComprehensiveCard'
And [Assertion] Verify 'QutClassOfBusinessDisplay' equals 'p:MotorPrivate' text
And [Assertion] Verify 'QutPolicyTypeDisplay' equals 'p:Comprehensive' text
And [Assertion] Verify 'QutStatusDisplay' equals 'p:Draft' text
And [Assertion] Verify 'QutValidityStatusDisplay' equals 'p:Valid' text
!-- Fill the mandatory Quotation Details
And [Assertion] Verify tab 'MainTab' is selected
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Assertion] Verify 'ErrorAlert' is enable
And [Assertion] Verify 'QutSourceErrorMsg' contains 'p:QutSourceEmptyErrorMsg' text
And [Assertion] Verify 'QutInsStDateErrorMsg' contains 'p:QutInsStDateEmptyErrorMsg' text
And [Assertion] Verify 'QutInsEdDateErrorMsg' contains 'p:QutInsEdDateEmptyErrorMsg' text
And [Navigation] I scroll to top of page
!-- And [Input] I click on 'MotorInformationTab'
!-- And [Assertion] Verify tab 'MotorInformationTab' is selected
!-- And [Assertion] Verify 'QutAwardedToErrorMsg' contains 'p:QutAwardedToEmptyErrorMsg' text
And [Input] I click on 'CalculationsTab'
And [Assertion] Verify tab 'CalculationsTab' is selected
And [Assertion] Verify 'QutCalculationBasisErrorMsg' contains 'p:QutCalculationBasisEmptyErrorMsg' text
And [Assertion] Verify 'QutCrCdErrorMsg' contains 'p:QutCrCdEmptyErrorMsg' text
And [Assertion] Verify 'QutQuotNoCreateText' input is empty
And [Input] I click on 'MainTab'
And [Assertion] Verify tab 'MainTab' is selected
!-- Fill the mandatory Quotation Details
And [Input] Select 'p:Direct' value from 'QutSourceCombobox' dropdown list
And [Assertion] Verify 'QutPaymentTermCombobox' contains 'p:Account' text
And [Assertion] Verify 'QutQuotDate' value equals sysdate
!-- And [Assertion] Verify 'QutQuotValidityDate' value equals sysdate plus '1' months minus '1' day
And [Input] Set 'QutInsStDateInput' value to 'sysdateformatted'
!-- And [Progress] I wait for '5' sec
!-- And [Input] I click on 'QutInsStDateInput'
!-- And [Input] I press Enter on 'QutInsStDateInput'
And [Progress] I wait for '6' sec
And [Assertion] Verify 'QutInsEdDateInput' value equals sysdate plus '1' years minus one day
And [Input] I click on 'MotorInformationTab'
And [Assertion] Verify tab 'MotorInformationTab' is selected
!-- And [Navigation] I scroll to 'QutAwardedToInput'
!-- And [Progress] I wait for '5' sec
And [Input] Set 'QutAwardedToInput' value to 'random5'
And [Input] I click on 'CalculationsTab'
And [Assertion] Verify tab 'CalculationsTab' is selected
And [Input] Select 'p:ProRata' value from 'QutCalculationBasisCombobox' dropdown list
And [Input] Select 'p:SaudiRiyals' value from 'QutCrCdCombobox' dropdown list
And [Progress] I wait for '3' sec
And [Assertion] Verify 'QutUnitPriceVal' equals '1' text
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Progress] I wait for '7' sec
And [Navigation] I scroll to 'SuccessfullyAlert'
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataSavedSuccessfully' text
And [Action] I get text from 'QutQuotNoCreateText' and save it as 'QuotationNo'
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill the mandatory Risk Details
And [Input] I click on 'RiskButton'
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Progress] I wait for '3' sec
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Assertion] Verify 'QurVehicleMakeErrorMsg' contains 'p:QurVehicleMakeEmptyErrorMsg' text
And [Assertion] Verify 'QurVehicleModelErrorMsg' contains 'p:QurVehicleModelEmptyErrorMsg' text
And [Assertion] Verify 'QurChassisNoErrorMsg' contains 'p:QurChassisNoEmptyErrorMsg' text
And [Assertion] Verify 'QurRegistrationNoErrorMsg' contains 'p:QurRegistrationNoEmptyErrorMsg' text
And [Assertion] Verify 'QurTypeOfBodyErrorMsg' contains 'p:QurTypeOfBodyEmptyErrorMsg' text
And [Assertion] Verify 'QurVehicleColorErrorMsg' contains 'p:QurVehicleColorEmptyErrorMsg' text
And [Input] I click on 'QurVehicleMakeCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:komatsu' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleModelCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Equipment' value from 'QurVehicleModelPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] Set 'QurChassisNoInput' value to 'nanotime'
And [Input] Set 'QurRegistrationNoInput' value to 'nanotime'
!-- And [Input] Select 'p:red' value from 'QurPlateColorCombobox' dropdown list
And [Input] I click on 'QurTypeOfBodyCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Sedan' value from 'QurTypeOfBodyPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleColorCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Green' value from 'QurVehicleColorPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
!-- And [Input] I click on 'CloseIconButton'
!-- And [Input] Set 'QurYearOfMakeInput' value to 'sysyear'
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
!-- And [Input] I click on 'CloseIconButton'
!-- And [Assertion] Verify 'QurYearOfMakeErrorMsg' contains 'p:QurYearOfMakeEmptyErrorMsg' text
!-- And [Input] I click on 'QurYearOfMakeInput'
!-- And [Input] I click on 'QurYearOfMakeTodayButton'
!-- And [Assertion] Verify 'QurYearOfMakeInput' input is not empty
!-- And [Navigation] I scroll to 'CreateButton'
!-- And [Input] I click on 'CreateButton'
And [Progress] I wait for '5' sec
And [Navigation] I scroll to 'SuccessfullyAlert'
And [Progress] I wait 'RiskButton' To be disappear
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:RowCreated' text
!--  Add for Vehicle Details and Driver Details
And [Input] I click on 'VehicleDetailsButton'
And [Input] Select 'TP' value from 'VehiclePremiumDetailsDDL' dropdown list
And [Assertion] Verify 'VehiclePremiumDetailsDDL' contains 'TP' text
And [Input] I fill IG 'F1_ig' with random values (combo/date/number) and save
And [Progress] I wait for '3' sec
And [Input] I fill IG 'F2_ig' with random values (combo/date/number) and save
And [Progress] I wait for '3' sec
And [Input] I fill IG 'F3_ig' with random values (combo/date/number) and save
And [Progress] I wait for '3' sec
And [Input] I fill IG 'F4_ig' with random values (combo/date/number) and save
And [Navigation] I scroll to top of page
And [Progress] I wait for '3' sec
And [Input] I click on 'RiskDetailsBackButton'
And [Progress] I wait for '3' sec
And [Input] I click on 'DriverDetailsButton'
And [Assertion] Verify tab 'ShowAllTab' is selected
And [Input] I click on 'PersonalInformationTab'
And [Assertion] Verify tab 'PersonalInformationTab' is selected
And [Assertion] Verify 'PersonalInformationHeader' contains 'p:PersonalInformation' text
And [Input] I click on 'ContactingAndAddressDetailsTab'
And [Assertion] Verify tab 'ContactingAndAddressDetailsTab' is selected
And [Assertion] Verify 'ContactingAndAddressDetailsHeader' contains 'p:ContactingAndAddressDetails' text
And [Input] I click on 'licenseInformationTab'
And [Assertion] Verify tab 'licenseInformationTab' is selected
And [Assertion] Verify 'licenseInformationHeader' contains 'p:licenseInformation' text
And [Input] I click on 'OtherInformationTab'
And [Assertion] Verify tab 'OtherInformationTab' is selected
And [Assertion] Verify 'OtherInformationHeader' contains 'p:OtherInformation' text
And [Input] I click on 'CancelButton'
And [Progress] I wait for '3' sec
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill the mandatory Risk Value Types
And [Navigation] I scroll to 'EditButton'
And [Input] I click on 'EditButton'
And [Assertion] Verify IG 'RiskCalculation_ig' row '1 - SI' Local/Amount can be filled
And [Input] I set SECOND IG 'RiskCalculation_ig' from first row '1 - SI' to random numbers between '1' and '50' for all enabled cells excluding DDL
And [Progress] I wait for '5' sec
And [Navigation] I scroll to 'SaveButton'
And [Input] I click on 'SaveButton'
And [Progress] I wait for '5' sec
And [Navigation] I scroll to top of page
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill in Quotation Value Types
And [Input] I click on 'EditButtonQuotationValue'
And [Navigation] I scroll to end of page
And [Assertion] Verify IG 'calc_ig' row '10 - Stamp Fees' Local/Amount can be filled
And [Input] I set IG 'calc_ig' from first row '1 - SI' to random numbers between '1' and '50' for all enabled cells excluding DDL
And [Progress] I wait for '10' sec
And [Input] I click on 'SaveButton'
And [Progress] I wait for '10' sec
And [Navigation] I scroll to top of page
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill in All Quotation Details
And [Assertion] Verify text of 'QutQuotNoAllQuotationDetails' contains saved value 'QuotationNo'
And [Navigation] I scroll to 'CustomerInfoButton'
And [Input] I click on 'CustomerInfoButton'
And [Input] Select 'p:ClientName' value from 'QurQutClientCombobox' dropdown list
And [Input] Select 'p:ClientName' value from 'QurQutClientNoCombobox' dropdown list
And [Input] I click on 'ApplyChangesButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
!-- Risk Quotation Button
And [Navigation] I scroll to end of page
And [Input] I click on 'RiskQuotationButton'
And [Assertion] Verify 'RiskTable' is visible
And [Input] I click on 'RiskDetailsCancelButton'
And [Progress] I wait for '5' sec
!-- Beneficiary Button
And [Navigation] I scroll to end of page
And [Input] I click on 'BeneficiaryButton'
And [Javascript] I switch to iframe 'BeneficiaryIframe'
And [Assertion] Verify 'BeneficiaryTable' is visible
And [Input] I click on 'BeneficiaryAddRow'
And [Input] I click on 'BeneficiarySerialRow'
And [Input] Set 'BeneficiarySerialRow' value to '1'
And [Input] I click on 'BeneficiaryRow'
And [Input] I click on 'BeneficiaryComboboxRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:ClientName' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'BeneficiaryIframe'
And [Progress] I wait for '5' sec
And [Input] I click on 'IdNumberRow'
And [Input] Set 'BeneficiaryIdNumberRow' value to '1'
And [Input] I click on 'NationalityRow'
And [Input] I click on 'NationalityRow'
!-- And [Input] I click on 'BeneficiaryNationalityRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:Afghani' value from 'BeneficiaryNationalityPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'BeneficiaryIframe'
And [Input] I click on 'ClassificationRow'
And [Input] I click on 'BeneficiaryClassificationRow'
And [Input] Select 'p:PerRegistrationNo' value from 'BeneficiaryClassificationRow' dropdown list
And [Input] I click on 'BeneficiarySaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
And [Input] I click on 'BeneficiaryIframeCloseButton'
!-- Covers Button
And [Input] I click on 'CoversButton'
And [Javascript] I switch to iframe 'CoversIframe'
And [Assertion] Verify 'CoversTable' is visible
And [Input] I click on 'CoverDeleteFirstRow' if present
And [Input] I click on 'CoversSaveButton'
And [Input] I click on 'CoversAddRow'
And [Input] I click on 'CoversSerialRow'
And [Input] Set 'CoversSerialRow' value to '1'
And [Input] I click on 'CategoryRow'
And [Input] I click on 'CoversCategoryComboboxRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:LimitationOfLiability' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'CoversIframe'
And [Input] I click on 'CoversRow'
And [Input] I click on 'CoversComboboxRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:AdditionDeletionAllowedOnProRataBasis' value from 'CoversPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'CoversIframe'
And [Input] I click on 'PremRow'
And [Input] Set 'CoversPremRow' value to '1'
And [Input] I click on 'CoversSaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
And [Input] I click on 'CloseButtonFromPopup'
!-- REI Plan
And [Input] I click on 'REIPlanButton'
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] I click on 'REIPlanSaveButton'
And [Assertion] Verify 'ReinsurancePlanErrorMsg' contains 'p:ReinsurancePlanEmptyErrorMsg' text
And [Assertion] Verify 'ReiMethodErrorMsg' contains 'p:ReiMethodEmptyErrorMsg' text
And [Input] I click on 'ReinsurancePlanCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:ReinsurancePlanValue' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] Select 'p:PolicyWise' value from 'ReiMethodCombobox' dropdown list
And [Input] I click on 'REIPlanSaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
And [Assertion] Verify 'IssueQuotationButton' is visible
And [Input] I click on 'IssueQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForIssue' text
And [Input] I click on 'IssueQuotationConfirmationButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Assertion] Verify 'IssueQuotationButton' is not visible
And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Issued'
And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Valid'
And [Assertion] Verify 'RejectQuotationButton' is visible
And [Input] I click on 'RejectQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForRejecting' text
And [Input] I click on 'CancelAlertButton'
And [Assertion] Verify 'ConvertToPolicyButton' is visible
And [Input] I click on 'ConvertToPolicyButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForCovertToPolicy' text
And [Input] I click on 'CancelAlertButton'
And [Assertion] Verify 'RejectQuotationButton' is visible
And [Input] I click on 'RejectQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForRejecting' text
And [Input] I click on 'RejectQuotationConfirmationButton'
And [Input] Set 'RejectDescriptionQuotationInput' value to 'random'
And [Input] I click on 'RejectButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Lost'
And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Invalid'
And [Assertion] Verify text of 'QutQuotNoAllQuotationDetails' equals saved value 'statemanager:QuotationNo'
And [Input] I click on 'CopyQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForCopy' text
And [Input] I click on 'CopyQuotationConfirmationButton'
And [Input] I click on 'ActionButton'
And [Input] I click on 'DataButton'
And [Input] I click on 'SortButton'
And [Input] Select 'ColumnDDL' value from 'p:number'
And [Input] Select 'DirectionDDL' value from 'p:descending'
And [Input] I click on 'SaveFilterButton'
And [Progress] I wait for '5' sec
And [Input] Set 'SearchQuotationInput' value to 'p:MotorPrivate'
And [Input] I press Enter on 'SearchQuotationInput'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'FirstClassOfBusInTable' equals 'p:MotorPrivate' text
And [Input] I click on 'EditPinButtonMultiRows'
!-- Add risk value
And [Navigation] I scroll to end of page
And [Input] I click on 'RiskQuotationButton'
And [Input] I click on 'RiskButton'
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Progress] I wait for '3' sec
And [Input] I click on 'QurVehicleMakeCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:komatsu' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleModelCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Equipment' value from 'QurVehicleModelPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] Set 'QurChassisNoInput' value to 'nanotime'
And [Input] Set 'QurRegistrationNoInput' value to 'nanotime'
And [Input] I click on 'QurTypeOfBodyCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Sedan' value from 'QurTypeOfBodyPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleColorCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Green' value from 'QurVehicleColorPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
!-- And [Input] I click on 'CloseIconButton'
!-- And [Input] Set 'QurYearOfMakeInput' value to 'sysyear'
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Progress] I wait for '3' sec
And [Assertion] Verify 'RiskTable' is visible
!-- Fill Calculations table
And [Input] I click on 'RiskDetailsCancelButton'
And [Progress] I wait for '5' sec
And [Navigation] I scroll to 'CalculationsButton'
And [Input] I click on 'CalculationsButton'
And [Input] I click on 'EditButtonQuotationValue'
And [Navigation] I scroll to end of page
And [Assertion] Verify IG 'calc_ig' row '10 - Stamp Fees' Local/Amount can be filled
And [Input] I set IG 'calc_ig' from first row '1 - SI' to random numbers between '1' and '50' for all enabled cells excluding DDL
And [Progress] I wait for '10' sec
And [Input] I click on 'SaveButton'
And [Progress] I wait for '10' sec
!-- REI Plan
And [Navigation] I scroll to 'REIPlanButton'
And [Input] I click on 'REIPlanButton'
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] I click on 'ReinsurancePlanCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:ReinsurancePlanValue' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] Select 'p:PolicyWise' value from 'ReiMethodCombobox' dropdown list
And [Input] I click on 'REIPlanSaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
!-- Customer Info
And [Navigation] I scroll to 'CustomerInfoButton'
And [Input] I click on 'CustomerInfoButton'
And [Input] Select 'p:ClientName2' value from 'QurQutClientCombobox' dropdown list
And [Input] Select 'p:ClientName2' value from 'QurQutClientNoCombobox' dropdown list
And [Input] I click on 'ApplyChangesButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Assertion] Verify 'IssueQuotationButton' is visible
And [Input] I click on 'IssueQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForIssue' text
And [Input] I click on 'IssueQuotationConfirmationButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Input] I click on 'CloseIconButton'
And [Assertion] Verify 'IssueQuotationButton' is not visible
And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Issued'
And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Valid'
And [Progress] I wait for '5' sec
And [Input] I click on 'ConvertToPolicyButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForCovertToPolicy' text
And [Input] I click on 'ConvertToPolicyConfirmationButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedConvertedToPolicySuccessfully' text
!-- And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Issued'
!-- And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Valid'
!-- Logout
And [Progress] I wait for '7' sec
And [Progress] I wait for '7' sec
Then [Input] I click on 'UserDDL'
And [Input] I click on 'LogoutButton'
And [Assertion] Verify 'loginButton' is present


Scenario:  TC_002_Apex TC2S2 Apex Test Case Motor - Motor Private - Comprehensive (US Dollars)

Given [Navigation] I navigate to 'p:amanApex'
And [Input] I login as 'p:adminUsername' with password 'p:password'
And [Progress] I wait for '5' sec
When [Input] I click on 'QuotationCardDashboard'
And [Assertion] Verify 'QuotationsTitle' equals 'Quotations' text
And [Input] I click on 'SPWaziriyahMrorrCard'
And [Input] I click on 'NewQuotationButton'
And [Input] I click on 'MotorCard'
And [Input] I click on 'MotorPrivateCard'
And [Input] I click on 'ComprehensiveCard'
And [Assertion] Verify 'QutStatusDisplay' equals 'p:Draft' text
And [Assertion] Verify 'QutValidityStatusDisplay' equals 'p:Valid' text
And [Assertion] Verify tab 'MainTab' is selected
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Assertion] Verify 'ErrorAlert' is enable
And [Assertion] Verify 'QutSourceErrorMsg' contains 'p:QutSourceEmptyErrorMsg' text
And [Assertion] Verify 'QutInsStDateErrorMsg' contains 'p:QutInsStDateEmptyErrorMsg' text
And [Assertion] Verify 'QutInsEdDateErrorMsg' contains 'p:QutInsEdDateEmptyErrorMsg' text
And [Navigation] I scroll to top of page
!-- And [Input] I click on 'MotorInformationTab'
!-- And [Assertion] Verify tab 'MotorInformationTab' is selected
!-- And [Assertion] Verify 'QutAwardedToErrorMsg' contains 'p:QutAwardedToEmptyErrorMsg' text
And [Input] I click on 'CalculationsTab'
And [Assertion] Verify tab 'CalculationsTab' is selected
And [Assertion] Verify 'QutCalculationBasisErrorMsg' contains 'p:QutCalculationBasisEmptyErrorMsg' text
And [Assertion] Verify 'QutCrCdErrorMsg' contains 'p:QutCrCdEmptyErrorMsg' text
And [Assertion] Verify 'QutQuotNoCreateText' input is empty
And [Input] I click on 'MainTab'
And [Assertion] Verify tab 'MainTab' is selected
!-- Fill the mandatory Quotation Details
And [Input] Select 'p:Direct' value from 'QutSourceCombobox' dropdown list
And [Assertion] Verify 'QutPaymentTermCombobox' contains 'p:Account' text
And [Assertion] Verify 'QutQuotDate' value equals sysdate
!-- And [Assertion] Verify 'QutQuotValidityDate' value equals sysdate plus '1' months minus '1' day
And [Input] Set 'QutInsStDateInput' value to 'sysdateformatted'
!-- And [Progress] I wait for '5' sec
!-- And [Input] I click on 'QutInsStDateInput'
!-- And [Input] I press Enter on 'QutInsStDateInput'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'QutInsEdDateInput' value equals sysdate plus '1' years minus one day
And [Input] I click on 'MotorInformationTab'
And [Assertion] Verify tab 'MotorInformationTab' is selected
!-- And [Navigation] I scroll to 'QutAwardedToInput'
!-- And [Progress] I wait for '5' sec
And [Input] Set 'QutAwardedToInput' value to 'random5'
And [Input] I click on 'CalculationsTab'
And [Assertion] Verify tab 'CalculationsTab' is selected
And [Input] Select 'p:ProRata' value from 'QutCalculationBasisCombobox' dropdown list
And [Input] Select 'p:USDollars' value from 'QutCrCdCombobox' dropdown list
And [Progress] I wait for '3' sec
And [Assertion] Verify 'QutUnitPriceVal' equals '3.75' text
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Progress] I wait for '7' sec
And [Navigation] I scroll to 'SuccessfullyAlert'
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataSavedSuccessfully' text
And [Action] I get text from 'QutQuotNoCreateText' and save it as 'QuotationNo'
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill the mandatory Risk Details
And [Input] I click on 'RiskButton'
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Progress] I wait for '3' sec
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Assertion] Verify 'QurVehicleMakeErrorMsg' contains 'p:QurVehicleMakeEmptyErrorMsg' text
And [Assertion] Verify 'QurVehicleModelErrorMsg' contains 'p:QurVehicleModelEmptyErrorMsg' text
And [Assertion] Verify 'QurChassisNoErrorMsg' contains 'p:QurChassisNoEmptyErrorMsg' text
And [Assertion] Verify 'QurRegistrationNoErrorMsg' contains 'p:QurRegistrationNoEmptyErrorMsg' text
And [Assertion] Verify 'QurTypeOfBodyErrorMsg' contains 'p:QurTypeOfBodyEmptyErrorMsg' text
And [Assertion] Verify 'QurVehicleColorErrorMsg' contains 'p:QurVehicleColorEmptyErrorMsg' text
And [Input] I click on 'QurVehicleMakeCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:komatsu' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleModelCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Equipment' value from 'QurVehicleModelPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] Set 'QurChassisNoInput' value to 'nanotime'
And [Input] Set 'QurRegistrationNoInput' value to 'nanotime'
!-- And [Input] Select 'p:red' value from 'QurPlateColorCombobox' dropdown list
And [Input] I click on 'QurTypeOfBodyCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Sedan' value from 'QurTypeOfBodyPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleColorCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Green' value from 'QurVehicleColorPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
!-- And [Input] I click on 'CloseIconButton'
!-- And [Input] Set 'QurYearOfMakeInput' value to 'sysyear'
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
!-- And [Input] I click on 'CloseIconButton'
!-- And [Assertion] Verify 'QurYearOfMakeErrorMsg' contains 'p:QurYearOfMakeEmptyErrorMsg' text
!-- And [Input] I click on 'QurYearOfMakeInput'
!-- And [Input] I click on 'QurYearOfMakeTodayButton'
!-- And [Assertion] Verify 'QurYearOfMakeInput' input is not empty
!-- And [Navigation] I scroll to 'CreateButton'
!-- And [Input] I click on 'CreateButton'
And [Progress] I wait for '5' sec
And [Navigation] I scroll to 'SuccessfullyAlert'
And [Progress] I wait 'RiskButton' To be disappear
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:RowCreated' text
!--  Add for Vehicle Details and Driver Details
And [Input] I click on 'VehicleDetailsButton'
And [Input] Select 'TP' value from 'VehiclePremiumDetailsDDL' dropdown list
And [Assertion] Verify 'VehiclePremiumDetailsDDL' contains 'TP' text
And [Input] I fill IG 'F1_ig' with random values (combo/date/number) and save
And [Progress] I wait for '3' sec
And [Input] I fill IG 'F2_ig' with random values (combo/date/number) and save
And [Progress] I wait for '3' sec
And [Input] I fill IG 'F3_ig' with random values (combo/date/number) and save
And [Progress] I wait for '3' sec
And [Input] I fill IG 'F4_ig' with random values (combo/date/number) and save
And [Navigation] I scroll to top of page
And [Progress] I wait for '3' sec
And [Input] I click on 'RiskDetailsBackButton'
And [Progress] I wait for '3' sec
And [Navigation] I scroll to 'DriverDetailsButton'
And [Input] I click on 'DriverDetailsButton'
And [Assertion] Verify tab 'ShowAllTab' is selected
And [Input] I click on 'PersonalInformationTab'
And [Assertion] Verify tab 'PersonalInformationTab' is selected
And [Assertion] Verify 'PersonalInformationHeader' contains 'p:PersonalInformation' text
And [Input] I click on 'ContactingAndAddressDetailsTab'
And [Assertion] Verify tab 'ContactingAndAddressDetailsTab' is selected
And [Assertion] Verify 'ContactingAndAddressDetailsHeader' contains 'p:ContactingAndAddressDetails' text
And [Input] I click on 'licenseInformationTab'
And [Assertion] Verify tab 'licenseInformationTab' is selected
And [Assertion] Verify 'licenseInformationHeader' contains 'p:licenseInformation' text
And [Input] I click on 'OtherInformationTab'
And [Assertion] Verify tab 'OtherInformationTab' is selected
And [Assertion] Verify 'OtherInformationHeader' contains 'p:OtherInformation' text
And [Input] I click on 'CancelButton'
And [Progress] I wait for '3' sec
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill the mandatory Risk Value Types
And [Navigation] I scroll to 'EditButton'
And [Input] I click on 'EditButton'
And [Assertion] Verify IG 'RiskCalculation_ig' row '1 - SI' Foreign/Foreign Amount can be filled
And [Input] I set SECOND IG 'RiskCalculation_ig' from first row '1 - SI' to random numbers between '1' and '50' for all enabled cells excluding DDL
And [Progress] I wait for '5' sec
And [Navigation] I scroll to 'SaveButton'
And [Input] I click on 'SaveButton'
And [Progress] I wait for '5' sec
And [Navigation] I scroll to top of page
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill in Quotation Value Types
And [Input] I click on 'EditButtonQuotationValue'
And [Navigation] I scroll to end of page
And [Assertion] Verify IG 'calc_ig' row '10 - Stamp Fees' Foreign/Foreign Amount can be filled
And [Input] I set IG 'calc_ig' from first row '1 - SI' to random numbers between '1' and '50' for all enabled cells excluding DDL
And [Progress] I wait for '10' sec
And [Input] I click on 'SaveButton'
And [Progress] I wait for '10' sec
And [Navigation] I scroll to top of page
And [Input] I click on 'NextButton'
And [Progress] I wait for '5' sec
!-- Fill in All Quotation Details
And [Assertion] Verify text of 'QutQuotNoAllQuotationDetails' contains saved value 'QuotationNo'
And [Navigation] I scroll to 'CustomerInfoButton'
And [Input] I click on 'CustomerInfoButton'
And [Input] Select 'p:ClientName' value from 'QurQutClientCombobox' dropdown list
And [Input] Select 'p:ClientName' value from 'QurQutClientNoCombobox' dropdown list
And [Input] I click on 'ApplyChangesButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
!-- Risk Quotation Button
And [Navigation] I scroll to end of page
And [Input] I click on 'RiskQuotationButton'
And [Assertion] Verify 'RiskTable' is visible
And [Input] I click on 'RiskDetailsCancelButton'
And [Progress] I wait for '5' sec
!-- Beneficiary Button
And [Navigation] I scroll to end of page
And [Input] I click on 'BeneficiaryButton'
And [Javascript] I switch to iframe 'BeneficiaryIframe'
And [Assertion] Verify 'BeneficiaryTable' is visible
And [Input] I click on 'BeneficiaryAddRow'
And [Input] I click on 'BeneficiarySerialRow'
And [Input] Set 'BeneficiarySerialRow' value to '1'
And [Input] I click on 'BeneficiaryRow'
And [Input] I click on 'BeneficiaryComboboxRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:ClientName' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'BeneficiaryIframe'
And [Progress] I wait for '5' sec
And [Input] I click on 'IdNumberRow'
And [Input] Set 'BeneficiaryIdNumberRow' value to '1'
And [Input] I click on 'NationalityRow'
And [Input] I click on 'NationalityRow'
!-- And [Input] I click on 'BeneficiaryNationalityRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:Afghani' value from 'BeneficiaryNationalityPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'BeneficiaryIframe'
And [Input] I click on 'ClassificationRow'
And [Input] I click on 'BeneficiaryClassificationRow'
And [Input] Select 'p:PerRegistrationNo' value from 'BeneficiaryClassificationRow' dropdown list
And [Input] I click on 'BeneficiarySaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
And [Input] I click on 'BeneficiaryIframeCloseButton'
!-- Covers Button
And [Input] I click on 'CoversButton'
And [Javascript] I switch to iframe 'CoversIframe'
And [Assertion] Verify 'CoversTable' is visible
And [Input] I click on 'CoverDeleteFirstRow' if present
And [Input] I click on 'CoversSaveButton'
And [Input] I click on 'CoversAddRow'
And [Input] I click on 'CoversSerialRow'
And [Input] Set 'CoversSerialRow' value to '1'
And [Input] I click on 'CategoryRow'
And [Input] I click on 'CoversCategoryComboboxRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:LimitationOfLiability' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'CoversIframe'
And [Input] I click on 'CoversRow'
And [Input] I click on 'CoversComboboxRow'
And [Javascript] I switch back to main content
And [Input] Select 'p:AdditionDeletionAllowedOnProRataBasis' value from 'CoversPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'CoversIframe'
And [Input] I click on 'PremRow'
And [Input] Set 'CoversPremRow' value to '1'
And [Input] I click on 'CoversSaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
And [Input] I click on 'CloseButtonFromPopup'
!-- REI Plan
And [Input] I click on 'REIPlanButton'
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] I click on 'REIPlanSaveButton'
And [Assertion] Verify 'ReinsurancePlanErrorMsg' contains 'p:ReinsurancePlanEmptyErrorMsg' text
And [Assertion] Verify 'ReiMethodErrorMsg' contains 'p:ReiMethodEmptyErrorMsg' text
And [Input] I click on 'ReinsurancePlanCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:ReinsurancePlanValue' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] Select 'p:PolicyWise' value from 'ReiMethodCombobox' dropdown list
And [Input] I click on 'REIPlanSaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
And [Assertion] Verify 'IssueQuotationButton' is visible
And [Input] I click on 'IssueQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForIssue' text
And [Input] I click on 'IssueQuotationConfirmationButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Assertion] Verify 'IssueQuotationButton' is not visible
And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Issued'
And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Valid'
And [Assertion] Verify 'RejectQuotationButton' is visible
And [Input] I click on 'RejectQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForRejecting' text
And [Input] I click on 'CancelAlertButton'
And [Assertion] Verify 'ConvertToPolicyButton' is visible
And [Input] I click on 'ConvertToPolicyButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForCovertToPolicy' text
And [Input] I click on 'CancelAlertButton'
And [Assertion] Verify 'RejectQuotationButton' is visible
And [Input] I click on 'RejectQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForRejecting' text
And [Input] I click on 'RejectQuotationConfirmationButton'
And [Input] Set 'RejectDescriptionQuotationInput' value to 'random'
And [Input] I click on 'RejectButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Lost'
And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Invalid'
And [Assertion] Verify text of 'QutQuotNoAllQuotationDetails' equals saved value 'statemanager:QuotationNo'
And [Input] I click on 'CopyQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForCopy' text
And [Input] I click on 'CopyQuotationConfirmationButton'
And [Input] I click on 'ActionButton'
And [Input] I click on 'DataButton'
And [Input] I click on 'SortButton'
And [Input] Select 'ColumnDDL' value from 'p:number'
And [Input] Select 'DirectionDDL' value from 'p:descending'
And [Input] I click on 'SaveFilterButton'
And [Progress] I wait for '5' sec
And [Input] Set 'SearchQuotationInput' value to 'p:MotorPrivate'
And [Input] I press Enter on 'SearchQuotationInput'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'FirstClassOfBusInTable' equals 'p:MotorPrivate' text
And [Input] I click on 'EditPinButtonMultiRows'
!-- Add risk value
And [Navigation] I scroll to end of page
And [Input] I click on 'RiskQuotationButton'
And [Input] I click on 'RiskButton'
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Progress] I wait for '3' sec
And [Input] I click on 'QurVehicleMakeCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:komatsu' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleModelCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Equipment' value from 'QurVehicleModelPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] Set 'QurChassisNoInput' value to 'nanotime'
And [Input] Set 'QurRegistrationNoInput' value to 'nanotime'
And [Input] I click on 'QurTypeOfBodyCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Sedan' value from 'QurTypeOfBodyPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
And [Input] I click on 'QurVehicleColorCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:Green' value from 'QurVehicleColorPopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'MotorRiskIframe'
!-- And [Input] I click on 'CloseIconButton'
!-- And [Input] Set 'QurYearOfMakeInput' value to 'sysyear'
And [Navigation] I scroll to 'CreateButton'
And [Input] I click on 'CreateButton'
And [Progress] I wait for '3' sec
And [Assertion] Verify 'RiskTable' is visible
!-- Fill Calculations table
And [Input] I click on 'RiskDetailsCancelButton'
And [Progress] I wait for '5' sec
And [Navigation] I scroll to 'CalculationsButton'
And [Input] I click on 'CalculationsButton'
And [Input] I click on 'EditButtonQuotationValue'
And [Navigation] I scroll to end of page
And [Assertion] Verify IG 'calc_ig' row '10 - Stamp Fees' Foreign/Foreign Amount can be filled
And [Input] I set IG 'calc_ig' from first row '1 - SI' to random numbers between '1' and '50' for all enabled cells excluding DDL
And [Progress] I wait for '10' sec
And [Input] I click on 'SaveButton'
And [Progress] I wait for '10' sec
!-- REI Plan
And [Navigation] I scroll to 'REIPlanButton'
And [Input] I click on 'REIPlanButton'
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] I click on 'ReinsurancePlanCombobox'
And [Javascript] I switch back to main content
And [Input] Select 'p:ReinsurancePlanValue' value from 'PopupLOVSearchBar' dropdown list
And [Javascript] I switch to iframe 'ReInsurancePlansIframe'
And [Input] Select 'p:PolicyWise' value from 'ReiMethodCombobox' dropdown list
And [Input] I click on 'REIPlanSaveButton'
And [Progress] I wait for '5' sec
And [Javascript] I switch back to main content
!-- Customer Info
And [Navigation] I scroll to 'CustomerInfoButton'
And [Input] I click on 'CustomerInfoButton'
And [Input] Select 'p:ClientName2' value from 'QurQutClientCombobox' dropdown list
And [Input] Select 'p:ClientName2' value from 'QurQutClientNoCombobox' dropdown list
And [Input] I click on 'ApplyChangesButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Assertion] Verify 'IssueQuotationButton' is visible
And [Input] I click on 'IssueQuotationButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForIssue' text
And [Input] I click on 'IssueQuotationConfirmationButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedSuccessfully' text
And [Progress] I wait for '5' sec
And [Input] I click on 'CloseIconButton'
And [Assertion] Verify 'IssueQuotationButton' is not visible
And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Issued'
And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Valid'
And [Progress] I wait for '5' sec
And [Input] I click on 'ConvertToPolicyButton'
And [Assertion] Verify 'AlertMsgBody' contains 'p:ConfirmationMsgForCovertToPolicy' text
And [Input] I click on 'ConvertToPolicyConfirmationButton'
And [Progress] I wait for '5' sec
And [Assertion] Verify 'SuccessfullyAlert' contains 'p:DataUpdatedConvertedToPolicySuccessfully' text
!-- And [Assertion] Verify text of 'StatusAllQuotationDetails' equals saved value 'p:Issued'
!-- And [Assertion] Verify text of 'ValidityStatusAllQuotationDetails' equals saved value 'p:Valid'
!-- Logout
And [Progress] I wait for '7' sec
Then [Input] I click on 'UserDDL'
And [Input] I click on 'LogoutButton'
And [Assertion] Verify 'loginButton' is present
